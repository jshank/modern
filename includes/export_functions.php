...
<td valign="top" bgcolor="#dddddd" style="padding:10px;">
	...
	{
		$eventurl = myEventRoot($events[$eid]);
	?>
		<div><a href="javascript:switchevent('<?php echo   $eventurl; ?>/zmEventImages.html');"><?=$eid?></a></div>
...
{     
    ...
    if (  !empty($eids) )
    {
		...
		if($exportImages) 
		{
			...
			if ( !($fp = fopen( $monitorPath."/".$file, "w" )) ) die( "Can't open event images export file '$monitorPath/$file'" );
...
function mygetEventPath( $event )
{
    ...
    if ( ZM_USE_DEEP_STORAGE )
        $eventPath = $thepath.'/'.ZM_DIR_EVENTS.'/'.$event['MonitorId'].'/'.strftime( "%y/%m/%d/%H/%M/%S", strtotime($event['StartTime']) );
    else
        $eventPath = $thepath.'/'.ZM_DIR_EVENTS.'/'.$event['MonitorId'].'/'.$event['Id'];
    return( $eventPath );
}
function myEventRoot( $event ) {
    if ( ZM_USE_DEEP_STORAGE )
